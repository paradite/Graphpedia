$(document).ready(() => {
  ...
  const height_separation = 50;
  // Varibale to control scale factorss
  const scale_factor_centerNode = 1.5;
  // Variable to control x-position of label
  const label_offset_x = 0;
  // Variable to control x-position of text
  ...
  let root = treeData = JSON.parse($('#json').val());
  // size of the diagram
  const viewerWidth = JSON.parse($('#tree-container').width());
  const viewerHeight = $(document).height() * 0.6;
...
*/
  ...
  function centerNode(source) {
    ...
    x = x * scale + viewerWidth / 7 + maxLabelLength;
    y = y * scale + viewerHeight / 2;
  ...
  function update(source) {
    ...
    childCount(0, root);
    const newHeight = d3.max(levelWidth) * height_separation; // 25 pixels per line
    ...
    nodeEnter.append('text')
      .attr('x', (d) => {
        ...
        // Move text if the term is in the middle
        if (!d.description) {
          return -50;
        }
      ...
      .attr('dy', (d) => {
        // Move text up if it is a term in the middle
        if (!d.description) {
          return '.35em';
        }
        return d.children || d._children ? '-.7em' : '.35em';
      })
      .attr('class', d => (d.description ? 'nodeText' : 'nodeRelationship'))
      .attr('text-anchor', (d) => {
        // Center the text if the node is a relationship
        if (!d.description) {
          return 'middle';
    ...
    node.select('text')
      .attr('x', (d) => {
        ...
        // Move text if the term is in the middle
        if (!d.description) {
          return -25 + label_offset_x;
        }
        return d.children || d._children ? d.name.length * 4 : 10;
      })
      .attr('text-anchor', (d) => {
        // Center the text if the node is a relationship
        if (!d.description) {
